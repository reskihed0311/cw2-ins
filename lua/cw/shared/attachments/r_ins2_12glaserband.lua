local att = {}
att.name = "laserband"
att.displayName = "Laser Band"
att.displayNameShort = "BND. Laser"
att.laserRange = 4096
...
if CLIENT then
	...
	function att:elementRender()
		local model = self.AttachmentModelsVM.laserband.ent
		...
		if not self.freeAimOn then
			if self.dt.State == CW_AIMING then
				angs = self.LaserBandAngAdjustAim
			end
		end
		
		angs = angs or self.LaserBandAngAdjust
		...
		local laserPos = pos + ang:Right() * self.LaserBandPosAdjust.x + ang:Forward() * self.LaserBandPosAdjust.y + ang:Up() * self.LaserBandPosAdjust.z
