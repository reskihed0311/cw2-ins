...
if CLIENT then
	att.displayIcon = surface.GetTextureID("vgui/atticons/rpeq15")
	att.description = {}
	...
	function att:elementRender()
		local model = self.AttachmentModelsVM.ranpeq.ent
		...
		if not self.freeAimOn then
			if self.dt.State == CW_AIMING then
				angs = self.RPEQAngAdjustAim
			end
		end
		
		angs = angs or self.RPEQLaserAngAdjust
		...
		local laserPos = pos + ang:Right() * self.RPEQPosAdjust.x + ang:Forward() * self.RPEQPosAdjust.y + ang:Up() * self.RPEQPosAdjust.z
