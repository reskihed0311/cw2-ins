local att = {}
att.name = "pistollaser"
att.displayName = "Pistol Laser"
att.displayNameShort = "SML LASER"
att.laserRange = 4096
...
if CLIENT then
	...
	function att:elementRender()
		local model = self.AttachmentModelsVM.pistollaser.ent
		...
		if not self.freeAimOn then
			if self.dt.State == CW_AIMING then
				angs = self.PistolLaserAngAdjustAim
			end
		end
		
		angs = angs or self.PistolLaserAngAdjust
		...
		local laserPos = pos + ang:Right() * self.PistolLaserPosAdjust.x + ang:Forward() * self.PistolLaserPosAdjust.y + ang:Up() * self.PistolLaserPosAdjust.z
